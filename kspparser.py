...
class FlowMode(Enum):
    NO_FLOW = 0
    STACK_PRIORITY_SEARCH = 1
    STAGE_PRIORITY_FLOW = 2
    ALL_VESSEL = 3   
 
class ResourceDefinition():
    def __init__(self,name,density,flowMode,transfer,isTweakable):
    
        self.name = name
        self.density = density
        self.flowMode = flowMode
        self.transfer = transfer
        self.isTweakable = isTweakable
    
def readResourceFiles():
    f = open(os.path.join(rpfn,'Resources//ResourcesGeneric.cfg'))
    lines = f.readlines()
    f.close()
...
def readPartFile(partFile):
    ...
    for m in modlst:
        ...
        #check to see if MODULE and curly brace are together
...
class Part(Asset):
    def __init__(self,partLines,**kwargs):
        ...
        self.mass = 1.
        self.author = None
        self.techRequired = None
        self.entryCost = 0
        self.cost = 0
        self.category = None
        self.title = None
        self.attachRules = None
    ...
    def mapDictToAttr(self,d):
        ...
        if'TechRequired' in d:
            self.techRequired = d['TechRequired']
        if 'entryCost' in d:
            self.entryCost = d['entryCost']
        if 'cost' in d:
            self.cost = d['cost']
        if 'category' in d:
        ...
        if 'title' in d:
            self.title = d['title']
        if 'attachRules' in d:
            self.attachRules = d['attachRules']
        if 'mass' in d:
            self.mass = 'mass'
